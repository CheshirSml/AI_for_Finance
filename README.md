...
```mermaid
flowchart TD
    A[❓ Пользовательский вопрос] --> B[🔁 Query Rewrite<br/>grok-3-mini]
    B --> C[📚 Hybrid Retrieval<br/>FAISS + BM25]
    C --> D[🧠 Reranker<br/>Qwen3-Reranker-4B]
    D --> E[🗜 Context Compression<br/>Mistral 24B]
    E --> F[💬 Answer Generation<br/>Mistral / LLaMA-3]
    F --> G{⚠️ Ошибка или слабый ответ?}
    G -->|Да| H[🩹 Fallback<br/>grok-3-mini]
    G -->|Нет| I[✅ Финальный ответ]
    H --> I
    I --> J[🧾 Логи и JSON-отчёты<br/>run_summary.json / events.jsonl / diagnostics_autotest.json]
```
